...
4. `python tools/ffmpeg.py mux` で各シーンの映像と音声を結合し、`python tools/ffmpeg.py concat` でシーン順に連結して、一時ファイル `output/video.mp4` を生成する。最終ファイル名への変更はCLIだけが行うため、`output/video.mp4` を `mv`、`rename`、削除、または別名保存してはいけない。
5. 一時出力ファイルが存在することを確認し、シーン数と一時ファイルの場所を報告する。
